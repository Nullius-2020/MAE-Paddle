...
from config_vit import update_config
...
def main():
    ...
    if config.MODEL.PRETRAINED:
        ...
        pretrained_dict = paddle.load(config.MODEL.PRETRAINED + '.pdparams')
        ...
        all_keys = list(pretrained_dict.keys())
        new_dict = OrderedDict()
        for key in all_keys:
            if key.startswith('backbone.'):
                new_dict[key[9:]] = pretrained_dict[key]
            elif key.startswith('encoder.'):
                new_dict[key[8:]] = pretrained_dict[key]
            else:
        ...
        if 'pos_embed' in pretrained_dict:
            ...
            if orig_size != new_size:
                ...
                pos_tokens = pos_tokens.transpose(perm=[0, 2, 3, 1]).flatten(1, 2)
