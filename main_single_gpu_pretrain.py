...
"""MAE Pretraining  using single GPU """
...
from mae import build_mae as build_model
...
def train(dataloader,
    ...
    for batch_id, data in enumerate(dataloader):
        image = data[0]
        #print(data[1].shape, data[1].flatten(1).shape)
        bool_masked_pos = data[1].astype(np.bool)
        ...
        with paddle.no_grad():
            ...
            unnorm_images = (image * std + mean).numpy()
            patch_size=16
            if normlize_target:
                images_squeeze = rearrange(unnorm_images, 'b c (h p1) (w p2) -> b (h w) (p1 p2) c', p1=patch_size, p2=patch_size)
                images_norm = (images_squeeze - images_squeeze.mean(axis=-2, keepdims=True)
            ...
            #print(images_patch.shape,bool_masked_pos.shape,bool_masked_pos[:64:196].shape)
            label = images_patch[bool_masked_pos].reshape([B, -1, C])
            #print(label)
...
def main():
    ...
    dataset_train = get_dataset(config, mode='pretrain')
    dataloader_train = get_dataloader(config, dataset_train, 'pretrain', False)
    # 3. Define criterion
