...
from transformer import PatchEmbedding,get_sinusoid_encoding_table,EncoderLayer,Identity
...
class MAEEncoder(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbedding(
            image_size=img_size, patch_size=patch_size, in_channels=in_chans, embed_dim=embed_dim)
        ...
        self.blocks = nn.LayerList([
            EncoderLayer(
                ...
                dropout=drop_rate, attention_dropout=attn_drop_rate, droppath=dpr[i])
        ...
        if use_learnable_pos_emb:
            self.pos_embed = paddle.framework.ParamAttr(initializer=nn.initializer.TruncatedNormal(std=.02))
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask is not None:
            ...
            #cls_tokens = self.cls_token.expand((x_vis.shape[0], -1, -1))
            #x_vis = paddle.concat((cls_tokens, x_vis), axis=1)
        #print(x_vis)
