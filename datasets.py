...
class ImageNet2012Dataset(Dataset):
    ...
    def __init__(self, file_folder, mode="train", transform=None,config=None):
        """Init ImageNet2012 Dataset with dataset file path, mode(train/val), and transform"""
        super(ImageNet2012Dataset, self).__init__()
        assert mode in ["pretrain","train", "val"]
        self.file_folder = file_folder
        self.transform = transform
        self.img_path_list = []
        self.label_list = []
        self.mode = mode
        self.masked_position_generator = RandomMaskingGenerator(config.MAE.WINDOW_SIZE, config.MAE.MASK_RADIO)
        if mode == "pretrain":
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "pretrain": 
           ...
           label = self.masked_position_generator()
        else:
            data = self.transform(data)
    ...
    if config.DATA.DATASET == "cifar10":
        ...
        if mode == 'pretrain':
            dataset = ImageNet2012Dataset(config.DATA.DATA_PATH,
                                          mode=mode,
                                          transform=get_train_transforms(config),config=config)
        elif mode == 'train':
            dataset = ImageNet2012Dataset(config.DATA.DATA_PATH,
                                          mode=mode,
                                          transform=get_train_transforms(config),config=config)
            ...
            dataset = ImageNet2012Dataset(config.DATA.DATA_PATH,
                                          mode=mode,
                                          transform=get_val_transforms(config),config=config)
    else:
